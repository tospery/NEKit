...
open class IPPacket {
    ...
    init?(packetData: Data) {
        ...
        guard let v = IPVersion(rawValue: vhl >> 4) else {
            nelog("Got unknown ip packet version \(vhl >> 4)")
            return nil
        }
        version = v
        ...
        guard totalLength == scanner.read16()! else {
            nelog("Packet length mismatches from header.")
            return nil
        }
        ...
        default:
            nelog("Can not parse packet header of type \(String(describing: transportProtocol)) yet")
            return nil
        }
    }
